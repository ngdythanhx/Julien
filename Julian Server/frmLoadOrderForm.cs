    ...
    public partial class frmLoadOrderForm : Form
    {
        ...
        private void frmLoadOrderForm_Load(object sender, EventArgs e)
        {
            ...
            if (Directory.Exists(pathOrders))
            {
                ...
                _lstSheetInfo = _lstSheetInfo.OrderBy(s => s.Employee).ToList();
                var lst = new List<string>() { "ALL" };
                var cbSource = _lstSheetInfo.GroupBy(s => s.Employee).Select(s => s.First().Employee).ToList();
                lst.AddRange(cbSource);
                cbEmployee.DataSource = lst;
        ...
        {
            var currentSource = dgvMain.DataSource as BindingList<SheetInfo>;
            ...
            {
                foreach (var s in currentSource.ToList())
                {
                    s.Checked = true;
                }
            }
        ...
        private async void LoadX()
        {
            ...
            {
                _frmReporter.SetDataSourceControl(LstOrder);
            }
            this.Close();
            /*}
            catch (Exception ex)
        ...
        private void cbEmployee_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                string curValue = cbEmployee.SelectedValue.ToString();
                if (curValue == "ALL")
                    dgvMain.DataSource = new SortableBindingList<SheetInfo>(_lstSheetInfo);
                else
                {

...
namespace ThuDamBao_TX
{
    public partial class Form1 : Form
    {
        ...
        {
            var a = txtInput.Text;
            List<DRData> list = new List<DRData>();
            ...
            {
                // dòng bắt đầu bằng DATABASE/LAB NO
            ...
            foreach (var line in lines.Skip(1))
            {
                ...
                DRData item = new DRData();

                item.Database = cols[0].Trim();

                item.Supplier = cols[1].Trim();

                item.DRDefectCode = cols[2].Trim();

                item.Shipment = cols[3].Trim();

                item.MaterialDescription = cols[4].Trim();

                item.MaterialID = cols[5].Trim();

                item.Qty = cols[6].Trim();

                item.Unit = cols[7].Trim();
                ...
                item.ModelName = cols[10].Trim();

                item.UsePurpose = cols[11].Trim();

                // 4 cột cuối
                item.FailTestItem = cols[cols.Length - 4].Trim();

                item.STD = cols[cols.Length - 3].Trim();

                item.TestResult = cols[cols.Length - 2].Trim();

                item.Retest = cols[cols.Length - 1].Trim();

                list.Add(item);
            }
            _lstMatName = list.Select(x => x.MaterialDescription ).ToList();
